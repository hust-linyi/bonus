...
class DataFolder(data.Dataset):
    def __init__(self, dir_list, post_fix, num_channels, data_transform=None, loader=img_loader, r=None):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.r is not None:
            target = sample[1]
            mask = ski_morph.dilation(target.squeeze().cpu().numpy()==1, footprint=ski_morph.disk(self.r))
            sample += (torch.tensor(mask.astype(float)).unsqueeze(0),)
